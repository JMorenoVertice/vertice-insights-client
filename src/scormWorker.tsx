...
const workerCode = () => {
  ...
  const getUserInfo = async () => {
    ...
    try {
      userData.hash = await generarHash(JSON.stringify(userData));
      self.postMessage(userData);
    } catch (error) {
      console.error("Error al generar el hash:", error);
      self.postMessage({ error: "Error al generar el hash" });
  ...
  const detectarDispositivo = () => 
    /mobile/i.test(navigator.userAgent) ? "Móvil" :
    /tablet/i.test(navigator.userAgent) ? "Tablet" : "Escritorio";

  const generarHash = async data => 
    Array.from(new Uint8Array(await crypto.subtle.digest(
      "SHA-256",
      new TextEncoder().encode(data)
    ))).map(b => b.toString(16).padStart(2, "0")).join("");

  self.onmessage = ({ data }) => actions[data]?.();
};
